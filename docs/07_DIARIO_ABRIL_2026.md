# 📔 Diário de Desenvolvimento - Abril 2026
...
---
## ??? 29 de Abril de 2026 - Unifica��o e Calibragem de Sugest�es

**Status:** Conclu�do e Sincronizado ?

### ?? Objetivos da Sess�o
- Unificar a quantidade de n�meros sugeridos em todos os sistemas de Estrelas/Especiais.
- Corrigir o erro de " Over-Prediction\ onde sistemas sugeriam 6 n�meros para jogos de pool reduzida (EuroDreams).
- Garantir que a compara��o de performance (Ranking) seja justa, com todos os sistemas a \jogar\ com o mesmo n�mero de sugest�es.

### ??? Problemas e Solu��es

#### 1. Inconsist�ncia na IA (Random Forest)
- **Problema:** A Random Forest sugeria fixamente 4 n�meros para todos os jogos, independentemente da pool.
- **Solu��o:** Atualizado o RandomForestSystem.ts para respeitar a nova regra universal: **6 (EM), 5 (TL), 3 (ED)**.

#### 2. Bug no Motor de Cache (Ranking Service)
- **Problema:** O cachePredictions utilizava a configura��o de n�meros principais (25/20) para fazer o slice das estrelas se n�o houvesse cuidado extra, ou usava fallbacks gen�ricos de 6 n�meros.
- **Solu��o:** Refatora��o do 
anking.ts para injetar o getPredictionCount correto durante a fase de gera��o de cache, garantindo que o EuroDreams nunca sugere n�meros fora do intervalo 1-5 e respeita o limite de 3 sugest�es.

#### 3. Calibragem de Pools Especiais
- **Defini��o Final:**
 - **EuroMilh�es:** 6 Estrelas (Pool de 12).
 - **Totoloto:** 5 N�meros da Sorte (Pool de 13).
 - **EuroDreams:** 3 N�meros de Sonho (Pool de 5).

### ?? Resultados
- Todos os sistemas (Estat�sticos e IA) agora produzem exatamente a mesma quantidade de n�meros por jogo.
- Eliminados n�meros \fantasma\ (ex: sugerir 8 ou 9 no EuroDreams).
- Ranking de Estrelas agora � 100% compar�vel e matematicamente honesto.


---
## 29 de Abril de 2024 - Sess�o de Noite: Transpar�ncia e Explicabilidade

**Status:** Conclu�do e Documentado ?

### ?? Objetivos da Sess�o
- Implementar um sistema de explicabilidade para todos os algoritmos de previs�o.
- Corrigir discrep�ncias visuais no EuroDreams e Totoloto.
- Resolver bloqueios de Build/Deploy na VPS.

### ??? Altera��es Realizadas

#### 1. Infraestrutura de Dados (Base de Dados)
- **Problema:** O site n�o tinha onde guardar as explica��es t�cnicas dos sistemas, for�ando textos fixos no c�digo.
- **Solu��o:** Adicionadas as colunas concept (Conceito) e logic (L�gica) a 3 tabelas fundamentais:
  - RankedSystem (Sistemas Gerais)
  - StarSystemRanking (Ranking de Estrelas)
  - SystemRanking (Ranking de N�meros)
- **Execu��o:** Criados e executados scripts de SQL Raw (ix-columns.ts, orce-fix.ts) para garantir a integridade da BD na VPS.

#### 2. Interface e Explicabilidade (UI/UX)
- **Implementa��o:** Criado um cart�o din�mico "?? Como Funciona Este Sistema" nas p�ginas de detalhe.
- **Dinamismo:** O componente deteta automaticamente o jogo e ajusta as cores (Amarelo, Verde, Rosa) e carrega o texto espec�fico da base de dados.
- **Sincroniza��o:** Executado um script de "Seed" massivo que mapeou explica��es para os 31 sistemas existentes, garantindo que varia��es como "Otimizado" ou "Stars" partilham a mesma l�gica base.

#### 3. Corre��o Cr�tica EuroDreams
- **Problema:** A interface de an�lise sugeria 6 estrelas e permitia n�meros at� 12, o que � imposs�vel no EuroDreams (Pool de 5).
- **Solu��o:** 
  - Parametrizado o getStarPrediction para aceitar um gameOverride.
  - Atualizadas as p�ginas de ranking para injetar o contexto correto do jogo.
  - O EuroDreams agora mostra corretamente o pool 1-5 e sugere apenas 3 n�meros de sonho.

#### 4. Resolu��o de Erros de Build (VPS)
- **Problema:** O TypeScript estava a bloquear o deploy por n�o reconhecer as novas colunas da base de dados durante a compila��o.
- **Solu��o:** Aplicado "Type Casting" estrat�gico nos componentes de servidor para permitir o build imediato enquanto o Prisma regenera os seus tipos internos na VPS.

### ? Estado Final
- **Transpar�ncia:** O utilizador agora entende a matem�tica por tr�s de cada palpite (Markov, Monte Carlo, Vortex, etc.).
- **Consist�ncia:** EuroDreams e Totoloto agora respeitam as suas pools reais.
- **Autonomia:** Novos sistemas adicionados � base de dados aparecer�o automaticamente explicados se os campos concept/logic forem preenchidos.
