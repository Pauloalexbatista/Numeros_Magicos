...
css = css.split('/* Add game-specific accent overrides */')[0];
css = css.replace(/\\n[data-game="EUROMILLIONS"]\\{ [^]*?\\}/g, '');
css = css.replace(/\\n[data-game="TOTOLOTO"]]\\{ [^]*?\\}/g, '');
css = css.replace(/\\n[data-game="EURODREAMS"]]\{ [^]*?\\}/g, '');
css = css.replace(/\\n[data-game="MEGASENA"]]\{ [^]*?\\}/g, '');

css = css.replace(/[data-game="EUROMILLIONS"] \{[^}]*\}/g, '')
     .replace(/[data-game="TOTOLOTO"] \{[^}]*\}/g, '')
     .replace(/[data-game="EURODREAMS"] \{[^}]*\}/g, '')
     .replace(/[data-game="MEGASENA"] \{[^}]*\}/g, '');
...
console.log('CSS overrides updated with Tailwind color vars');