...
export default async function TotolotoSystemDetailsPage({ params }: Props) {
    ...
    console.log('[DEBUG] Totoloto System Page:', { encodedName, systemName });
    ...
    uniquePerformances.forEach(p => {
        const hits = Math.min(5, Math.max(0, p.hits)); // Totoloto also matches up to 5 main numbers + 1 lucky number (usually separate, but systemPerformance mostly tracks numbers hits)
        distribution[hits]++;
        totalHits += hits;
    });
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 p-6">
            <div className="container mx-auto space-y-8 max-w-5xl">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex gap-2">
                        <Link
                            ...
                            className="px-4 py-2 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white rounded-lg font-medium transition-all shadow-lg hover:shadow-xl flex items-center gap-2"
                        >
                            📊 Análise Histórica
                        </Link>
                        {antiSystemExists && (
                            <Link
                                ...
                                className="px-4 py-2 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white rounded-lg font-medium transition-all shadow-lg hover:shadow-xl flex items-center gap-2"
                            >
                                🔄 Comparar Sistemas
                            </Link>
                        )}
                ...
                {/* 🔮 NEXT PREDICTION CARD (Highlighted) - TOTOLOTO THEME */}
                <Card className="p-8 bg-gradient-to-br from-orange-900/40 to-amber-900/20 border-orange-500/30 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-4 opacity-10">
                        <span className="text-9xl">🔮</span>
                    </div>

                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <h2 className="text-xl font-bold text-orange-100 flex items-center gap-2 shrink-0">
                        ...
                        {nextPrediction && nextPrediction.length > 0 ? (
                            nextPrediction.map((num: number) => (
                                <div key={num} className="relative group">
                                    <div className="absolute inset-0 bg-orange-400/30 rounded-full blur-md group-hover:blur-lg transition-all"></div>
                                    <div className="relative w-12 h-12 flex items-center justify-center bg-gradient-to-br from-orange-400 to-amber-600 rounded-full text-lg font-black text-white shadow-xl border-2 border-orange-300">
                                        {num}
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="text-orange-200/50 italic">Calculando previsão...</div>
                        )}
                    </div>
                                        ...
                                        <tr key={pred.id} className="hover:bg-slate-800/30 transition-colors">
                                                    ...
                                                    {predicted.map((n: number) => {
                                                        ...
                                                        return (
                                                            <span key={n} className={`
                                                                ...
                                                                ${isHit ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-slate-800 text-slate-500'}
                                                            `}>
                                                                {n}
                                                            </span>
                                                        );
                                            ...
                                            <td className="p-4 text-center">
                                                <span className={`
                                                    ...
                                                    ${pred.hits >= 3 ? 'bg-orange-500/20 text-orange-400' :
                                                        pred.hits >= 1 ? 'bg-amber-500/20 text-amber-400' :
