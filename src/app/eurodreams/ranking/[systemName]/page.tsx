...
export default async function EuroDreamsSystemDetailsPage({ params }: Props) {
    ...
    console.log('[DEBUG] EuroDreams System Page:', { encodedName, systemName });
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 p-6">
            <div className="container mx-auto space-y-8 max-w-5xl">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex gap-2">
                        <Link
                            ...
                            className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-lg font-medium transition-all shadow-lg hover:shadow-xl flex items-center gap-2"
                        >
                            📊 Análise Histórica
                        </Link>
                        {antiSystemExists && (
                            <Link
                                ...
                                className="px-4 py-2 bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 text-white rounded-lg font-medium transition-all shadow-lg hover:shadow-xl flex items-center gap-2"
                            >
                                🔄 Comparar Sistemas
                            </Link>
                        )}
                ...
                {/* 🔮 NEXT PREDICTION CARD (Highlighted) - DREAM THEME */}
                <Card className="p-8 bg-gradient-to-br from-purple-900/40 to-indigo-900/20 border-purple-500/30 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-4 opacity-10">
                        <span className="text-9xl">🔮</span>
                    </div>

                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <h2 className="text-xl font-bold text-purple-100 flex items-center gap-2 shrink-0">
                        ...
                        {nextPrediction && nextPrediction.length > 0 ? (
                            nextPrediction.map((num: number) => (
                                <div key={num} className="relative group">
                                    <div className="absolute inset-0 bg-purple-400/30 rounded-full blur-md group-hover:blur-lg transition-all"></div>
                                    <div className="relative w-12 h-12 flex items-center justify-center bg-gradient-to-br from-purple-400 to-indigo-600 rounded-full text-lg font-black text-white shadow-xl border-2 border-purple-300">
                                        {num}
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="text-purple-200/50 italic">Calculando previsão...</div>
                        )}
                    </div>
                                                    ...
                                                    {predicted.map((n: number) => {
                                                        ...
                                                        return (
                                                            <span key={n} className={`
                                                                ...
                                                                ${isHit ? 'bg-purple-500 text-white shadow-lg shadow-purple-500/20' : 'bg-slate-800 text-slate-500'}
                                                            `}>
                                                                {n}
                                                            </span>
                                                        );
