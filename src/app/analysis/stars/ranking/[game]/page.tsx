...
export default async function StarRankingPage({ params, searchParams }: PageProps) {
    ...
    if (timeframe === 'historical') {
        ...
        subtitle = 'Análise Histórica Completa (Desde 2004)';
    } else {
        // Last 100 or Last 20
        rankings = await getStarRankingMetrics(gameType, timeframe);

        if (timeframe === 'last20') {
            subtitle = 'Performance (Últimos 20 Sorteios)';
        } else {
            subtitle = 'Performance (Últimos 100 Sorteios)';
        }
    }
        ...
        <div className={`min-h-screen ${theme.bg} p-6 pb-24 font-sans`}>
            <div className="container mx-auto space-y-8 max-w-5xl">
                ...
                {timeframe === 'historical' && (
                    <TopStarSystemsAnalysis data={yearlyAnalysis} game={gameType} themeMode="light" />
                )}
