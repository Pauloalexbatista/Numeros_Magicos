...
/**
 * SEED PRODUCTION DATABASE (SAFE VERSION)
 * Uses isolated Prisma Client to bypass file locks.
 */
...
async function importTable(tableName: string, modelName: string, batchSize = 1000) {
    ...
    // Use stream or load full? With 200k records, assume export worked, so load full is OK if Node allows.
...
async function main() {
    ...
    console.log(`🚀 Starting Production Database Seed (SAFE) - ${isQuick ? 'QUICK' : 'COMPLETE'} SYNC...`);
