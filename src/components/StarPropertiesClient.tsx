...
interface StarPropertiesStats {
    parity: {
        ...
        '2I': number;
        '1P1I': number;
        '1P'?: number; // 1 star game
        '1I'?: number; // 1 star game
    };
    highLow: {
        '2H': number;
        '2L': number;
        '1H1L': number;
        '1H'?: number; // 1 star game
        '1L'?: number; // 1 star game
    };
    primes: {
...
export function StarPropertiesClient({ stats, game = 'EUROMILLIONS' }: StarPropertiesClientProps) {
    const getPercentage = (val: number = 0) => Math.round((val / stats.totalDraws) * 100);

    const isTotoloto = game === 'TOTOLOTO';
    const isEuroDreams = game === 'EURODREAMS';
    const hasSingleStar = isTotoloto || isEuroDreams;
    const themeColor = isTotoloto ? 'emerald' : isEuroDreams ? 'rose' : 'yellow';

    const highLabel = isEuroDreams ? 'Altos (3-5)' : isTotoloto ? 'Altos (7-13)' : 'Altos (7-12)';
    const lowLabel = isEuroDreams ? 'Baixos (1-2)' : 'Baixos (1-6)';

    return (
        <Card className={`p-6 bg-gradient-to-br from-${themeColor}-50 to-${themeColor}-100 dark:from-${themeColor}-950 dark:to-${themeColor}-900 border-${themeColor}-200 dark:border-${themeColor}-800 backdrop-blur-sm col-span-1 md:col-span-2`}>
            <h2 className={`text-xl font-bold text-${themeColor}-800 dark:text-${themeColor}-200 mb-4 flex items-center gap-2`}>
                🧬 Propriedades Combinadas {hasSingleStar ? (isTotoloto ? 'do N. Sorte' : 'do N. Sonho') : 'das Estrelas'}
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                ...
                {!hasSingleStar && (
                    <div className={`bg-white/50 dark:bg-black/20 p-4 rounded-lg border border-${themeColor}-200/50 dark:border-${themeColor}-700/30`}>
                        ...
                        <div className="flex flex-col items-center justify-center h-32">
                            <div className={`text-3xl font-bold text-${themeColor}-900 dark:text-${themeColor}-100 mb-1`}>
                                {getPercentage(stats.consecutive.yes)}%
                            </div>
                            <div className={`text-xs text-${themeColor}-700 dark:text-${themeColor}-400 mb-4`}>Saem Seguidas (Ex: 3-4)</div>

                            <div className={`w-full bg-${themeColor}-200 dark:bg-${themeColor}-800 h-2 rounded-full overflow-hidden`}>
                                <div className="bg-green-500 h-full" style={{ width: `${getPercentage(stats.consecutive.yes)}%` }} />
                            </div>
                            <div className={`flex justify-between w-full mt-1 text-[10px] text-${themeColor}-600 dark:text-${themeColor}-500`}>
                                <span>Sim</span>
                ...
                <div className={`bg-white/50 dark:bg-black/20 p-4 rounded-lg border border-${themeColor}-200/50 dark:border-${themeColor}-700/30`}>
                    ...
                    <div className="flex flex-col items-center justify-center h-32">
                        ...
                        <div className={`text-xs text-${themeColor}-700 dark:text-${themeColor}-400`}>Média da Soma</div>

                        <div className="flex gap-4 mt-4 text-xs">
                            <div className="text-center">
                                <div className={`font-bold text-${themeColor}-900 dark:text-${themeColor}-100`}>{stats.sum.min}</div>
                                <div className={`text-${themeColor}-600 dark:text-${themeColor}-500`}>Mín</div>
                            </div>
                            <div className="text-center">
                                <div className={`font-bold text-${themeColor}-900 dark:text-${themeColor}-100`}>{stats.sum.max}</div>
                                <div className={`text-${themeColor}-600 dark:text-${themeColor}-500`}>Máx</div>
                            </div>
                        </div>
                    </div>
                </div>
