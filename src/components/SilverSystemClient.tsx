...
export default function SilverSystemClient() {
    ...
    useEffect(() => {
        async function loadCachedData() {
            try {
                ...
                // 1. Fetch Silver System Prediction from Cache
                const silverResponse = await fetch('/api/predictions/latest?system=Silver System');
                const silverData = await silverResponse.json();

                if (silverData.numbers && silverData.numbers.length > 0) {
                    setFinalPrediction(silverData.numbers);
                } else {
                    console.warn('No cached prediction for Silver System');
                }

                // 2. Fetch Ranking to identify Top 6 Contributors
                const rankingResponse = await fetch('/api/ranking');
                if (!rankingResponse.ok) throw new Error('Falha ao carregar ranking');
                const rankingData = await rankingResponse.json();

                const top6 = rankingData.ranking.slice(0, 6);

                // 3. Fetch Cached Predictions for Top 6
                const contribs: SystemContribution[] = [];

                await Promise.all(top6.map(async (r: any) => {
                    try {
                ...
                if (silverData.numbers.length === 0 && Object.keys(votes).length > 0) {
