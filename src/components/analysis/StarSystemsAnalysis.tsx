...
export function StarSystemsAnalysis({ data }: StarSystemsAnalysisProps) {
    ...
    return (
        <Card className="p-6 bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 backdrop-blur-sm shadow-sm">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-zinc-100 dark:border-zinc-800 text-zinc-500 text-[10px] uppercase font-bold tracking-widest">
                            ...
                            <th className="py-3 px-4 text-center text-zinc-600 dark:text-zinc-400">1 Estrela (1★) 💰</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-50 dark:divide-zinc-900">
                        {currentStats.map((stat, index) => (
                            <tr key={stat.systemName} className="hover:bg-yellow-50/50 dark:hover:bg-yellow-900/5 transition-colors group">
                                <td className="py-4 px-4">
                                    <div className={`
                                        ...
                                        ${index === 0 ? 'bg-yellow-500 text-black shadow-lg shadow-yellow-500/10' :
                                            index === 1 ? 'bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400' :
                                ...
                                <td className="py-4 px-4">
                                    <span className="font-bold text-zinc-800 dark:text-zinc-200 group-hover:text-yellow-600 dark:group-hover:text-yellow-400 transition-colors">
                                        {stat.systemName}
                                    </span>
                                </td>
