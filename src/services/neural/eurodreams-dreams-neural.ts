...
const PREDICTION_COUNT = 1; // EuroDreams tem 1 Sonho
...
export async function trainEuroDreamsDreams(options: NeuralTrainingOptions = {}): Promise<{ success: boolean; accuracy?: number; message: string }> {
    try {
        await NeuralPersistenceService.reportProgress('LSTM', 'EURODREAMS', 'SONHOS', 5);
        ...
        await model.fit(tensorData.xs, tensorData.ys, {
            ...
            callbacks: {
                onEpochEnd: (epoch, logs) => {
                    ...
                    NeuralPersistenceService.reportProgress('LSTM', 'EURODREAMS', 'SONHOS', progress);
                    if (logs && epoch % 20 === 0) {
                        finalLoss = logs.loss;
        ...
        await NeuralPersistenceService.reportProgress('LSTM', 'EURODREAMS', 'SONHOS', 100);
