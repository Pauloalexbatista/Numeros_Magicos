    ...
    analyzeResonance(history: Draw[]): { star: number, score: number }[] {
        ...
        for (let candidate = 1; candidate <= maxStar; candidate++) {
            ...
            for (let i = history.length - 1; i >= 0; i--) {
                ...
                const drawnStars = (typeof draw.stars === "string" ? (typeof draw.stars === "string" ? (typeof draw.stars === "string" ? JSON.parse(draw.stars) : draw.stars) : draw.stars) : draw.stars as unknown) as number[];
