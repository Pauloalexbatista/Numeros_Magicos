...
const unifiedMegaSena = `[GameType.MEGASENA]: {
...
for (const file of files) {
    if (fs.existsSync(file)) {
        ...
        // Match the MEGASENA block until the next GameType block
        content = content.replace(/\[GameType\.MEGASENA\]:\s*\{[\s\S]*?\},\s*\[GameType\.EUROMILLIONS\]:/, unifiedMegaSena + "\n    [GameType.EUROMILLIONS]:");
        fs.writeFileSync(file, content, 'utf8');
        console.log("Fixed unified MegaSena in " + file);
    }
}
